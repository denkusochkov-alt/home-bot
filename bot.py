...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "/все — все траты\n"
        "/мои — мои траты\n"
        "/баланс — кто сколько должен\n"
        "/итог — итог текущего месяца\n"
        "/поиск [запрос] — поиск по тратам\n"
        "/список — список покупок\n"
        "/купить [товар] — добавить в список\n"
        "/купил [номер] — отметить куплено\n"
        "/отмена — удалить последнюю трату\n"
    )
...
def main():
    ...
    app.add_handler(CommandHandler("все", cmd_all))
    app.add_handler(CommandHandler("мои", cmd_mine))
    app.add_handler(CommandHandler("баланс", cmd_balance))
    app.add_handler(CommandHandler("итог", cmd_summary))
    app.add_handler(CommandHandler("поиск", cmd_search))
    app.add_handler(CommandHandler("список", cmd_shopping_list))
    app.add_handler(CommandHandler("купить", cmd_add_to_list))
    app.add_handler(CommandHandler("купил", cmd_mark_bought))
    app.add_handler(CommandHandler("отмена", cmd_undo))
    app.add_handler(CommandHandler("автоотчет", cmd_setup_monthly))
